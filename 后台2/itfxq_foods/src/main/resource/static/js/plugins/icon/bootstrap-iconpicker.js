...
+function ($) { "use strict";
    ...
    Iconpicker.prototype.createButtonBar = function(){
        ...
        for(var i = 0; i < op.cols; i++){
            var btn = $('<button class="btn ' + op.arrowClass + '"><span class="mdi"></span></button>');
            var td = $('<td class="text-center"></td>');
            if(i == 0 || i == op.cols - 1){
                btn.val((i==0) ? -1: 1);
                btn.addClass((i==0) ? 'btn-previous': 'btn-next');
                btn.find('span').addClass( (i == 0) ? 'mdi-arrow-left': 'mdi-arrow-right');
                td.append(btn);
                tr.append(td);
            }
    ...
    $.fn.iconpicker = function (option, params) {
        return this.each(function () {
            ...
            if (typeof option == 'string') data[option](params)
                ...
                var op = data.options;
                var ic = (op.iconset == 'fontawesome') ? 'fa' : 'mdi';
                op = $.extend(op, {
                    ic: ic,
